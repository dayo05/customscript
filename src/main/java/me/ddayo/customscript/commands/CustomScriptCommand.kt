...
object CustomScriptCommand: CommandHandler.ICommand {
        ...
        dispatcher.register(Commands.literal("cs")
                                        ...
                                        .then(Commands.argument("value", StringArgumentType.string())
                                                .executes {
                                                    GameProfileArgument.getGameProfiles(it, "player").map {profile -> ServerLifecycleHooks.getCurrentServer().playerList.getPlayerByUUID(profile.id)!!}.forEach {p ->
                                                        ServerDataHandler.updateData(p, it.getArgument("value name", String::class.java), it.getArgument("value", String::class.java))
                                                    }
                                                    1
                                                }))))
                .then(Commands.literal("internal-value")
                        .then(Commands.argument("value name", StringArgumentType.string())
                                .then(Commands.argument("value", StringArgumentType.string())
                                        .executes {
                                            ServerDataHandler.updateInternalData(it.source, it.getArgument("value name", String::class.java), it.getArgument("value", String::class.java))
                                            1
                                        }))))
    }
    ...
    private fun execute(it: CommandContext<CommandSource>): Int {
        ...
        if(CustomScript.isClient)
            ...
            for(p in players) {
                ServerDataHandler.playerScriptState[p.uniqueID] = ServerDataHandler.ScriptStateData(script, p.posX, p.posY, p.posZ)
                values.forEach {
                    if(ServerDataHandler.dynamicData[p.uniqueID]!!.contains(it.key))
                        CustomScript.network.sendTo(UpdateValueNetworkHandler(it.key, ServerDataHandler.dynamicData[p.uniqueID]!![it.key]!!), p.connection.networkManager, NetworkDirection.PLAY_TO_CLIENT)
                    else if(ServerDataHandler.internalData.contains(it.key))
                        CustomScript.network.sendTo(UpdateValueNetworkHandler(it.key, ServerDataHandler.internalData[it.key]!!), p.connection.networkManager, NetworkDirection.PLAY_TO_CLIENT)
                }
